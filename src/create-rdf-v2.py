...
# python create-rdf-v2.py out-jsonl-test/out-2-2.jsonl out-2.ttl
# INPUT_FILE = args.input
INPUT_FILE = "out-test/out-2-2.jsonl"
# OUTPUT_FILE = f"out/{args.output}.ttl"
# OUTPUT_FILE_2 = f"out/{args.output}.nt"
OUTPUT_FILE = f"out-test/out-2.ttl"
# OUTPUT_FILE_2 = f"out-nt/{args.output}.nt"

# ==== NAMESPACES ====
ZBMATH = Namespace("https://zbmath.org/")
# MSC = Namespace("http://msc2010.org/resources/MSC/2010/")
MSC = Namespace("http://msc2020.org/")
...
ONTOLOGY = URIRef("https://zbmath.org/ontology")

g.bind("owl", OWL)

g.add((ONTOLOGY, RDF.type, OWL.Ontology))
g.add((ONTOLOGY, DCTERMS.title,
       Literal("zbMATH Open Knowledge Graph Ontology")))
g.add((ONTOLOGY, OWL.versionInfo,
       Literal("1.0")))
...
# ==== LOAD JSONL AND BUILD CLEAN RDF ====
        ...
        try:
            ...
            if valid_author_ids:
                for i, aid in enumerate(author_ids):
                    ...
                    g.add((author_uri, SCHEMA.authorOf, record_uri))
            ...
            if classifications:
                for msc_code in classifications:
                    ...
                    parent = msc_info.get("parent")
                    parent_uri = MSC[parent]
                    g.add((msc_uri, SKOS.broader, parent_uri))
                    g.add((parent_uri, SKOS.narrower, msc_uri))
            ...
            if zbl_id and zbl_id != "None":
                ...
                g.add((record_uri, DCTERMS.identifier, Literal(zbl_id)))

            doi = data.get("doi", [None])[0]
            if doi and doi != "None":
                doi_uri = URIRef(f"https://doi.org/{doi}")
                g.add((record_uri, SCHEMA.identifier, doi_uri))
                g.add((record_uri, SCHEMA.sameAs, doi_uri))
                # g.add((record_uri, DCTERMS.identifier, Literal(doi)))
            ...
            if any([review_text, reviewer_id, review_sign]) and review_text != "None":
                # review_node = BNode()
                review_uri = URIRef(f"https://zbmath.org/review/{doc_id}")
                g.add((record_uri, SCHEMA.review, review_uri))
                g.add((review_uri, RDF.type, SCHEMA.Review))
                g.add((review_uri, SCHEMA.itemReviewed, record_uri))

                # Review body
                if review_text and review_text != "None":
                    g.add((review_uri, SCHEMA.reviewBody, Literal(review_text)))

                # Review language
                if review_lang and review_lang != "None":
                    g.add((review_uri, SCHEMA.inLanguage, Literal(review_lang)))

                # Review type
                if review_type and review_type != "None":
                    g.add((review_uri, SCHEMA.reviewAspect, Literal(review_type)))
                ...
                if reviewer_id and reviewer_id != "None":
                    ...
                    g.add((review_uri, SCHEMA.reviewer, reviewer_uri)) 
                    g.add((review_uri, DCTERMS.creator, reviewer_uri))
                    ...
                    if review_sign and review_sign != "None":
                        ...
                        g.add((reviewer_uri, RDFS.label, Literal(review_sign))) 
                    else:
                        g.add((reviewer_uri, SCHEMA.name, Literal(reviewer_id)))
                        g.add((reviewer_uri, RDFS.label, Literal(reviewer_id))) 
            ...
            if link_data:
                ...
                for l in links:
                    if not l or l == "None":
                        continue
                    if l.startswith("https://doi.org/") or l.startswith("http://doi.org/"):
                        g.add((record_uri, SCHEMA.sameAs, URIRef(l)))
                    else:
